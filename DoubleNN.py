...
p = PLE(game, fps=60, force_fps=True, display_screen=False,
...
def get_next_epsilon(x):  # CHANGE THIS AFTER YOU FINISH
    return max(0.001, x - 1 / (nr_steps_per_episode * 3))
    ...
    for step in range(nr_steps_per_episode):
        ...
        if (len(current_NN_states) >= min_nr_training_states and step % 20 == 0):
            ...
            for state_nr in random.sample(range(0, min_nr_training_states), nr_states_for_training):
                ...
                target_states[len(target_states) - 1][action_no] = current_NN_states[state_nr][3] # adding reward
                Qs_for_state_prime = target_network.result(current_NN_states[state_nr][2].reshape(1, first_layer_size))
                # computing Q-s for state prime to extract argmax
                for it in Qs_for_state_prime[0]:
                    if math.isnan(it):
                        print("NAN ERROR, NAN ERROR!!!")
                        exit(0) # just a test in case NN crashes?
                states = list(range(len(actions)))
                states.sort(key=lambda i: Qs_for_state_prime[0][i], reverse=True)
                action = 0
                for i in states:
                    if validate_action(state, actions[i]):
                        action_no = i
                        break
                #now we have a valid action(0 -> 3)
                Qs_state_prime_NN = network.result(current_NN_states[state_nr][2].reshape(1, first_layer_size))
                Qs_state_prime_Target_NN = target_network.result(
                    current_NN_states[state_nr][2].reshape(1, first_layer_size))

                state_prime_best_action = np.argmax(Qs_for_state_prime[0])
                target_states[len(target_states) - 1][action_no] += \
                    gama * min(Qs_state_prime_NN[0][action], Qs_state_prime_Target_NN[0][action])
                # TARGET = Reward + gama * min(Q(s(t + 1))[argmax(Q_Target(s(t + 1))]

            network.train(np.array(training_states), np.array(target_states))
            target_network.model.set_weights(theta_update_target_weights * np.array(network.model.get_weights()) +
                                             (1 - theta_update_target_weights) * np.array(target_network.model.get_weights()))
